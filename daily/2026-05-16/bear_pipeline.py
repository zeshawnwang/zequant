...
使用和oos_pipeline.py完全相同的bt()函数、信号构建方法、策略定义。
"""
...
BEAR_DIR=os.path.join(os.path.dirname(__file__),"bear_verify");os.makedirs(BEAR_DIR,exist_ok=True)
...
def load():
    ...
    df=db.get_factors(start_date="2021-06-01",end_date="2022-12-30",factor_names=available,with_close=True)
...
def main():
    ...
    logger.info("="*60)
    logger.info(f"2022熊市验证 — {len(strategies)+len(combo_strategies)}策略")
    logger.info("="*60)

    # ── 单策略测试 ──
    for sname, sig, rf, tn, pr, mhd in strategies:
        logger.info(f"\n--- {sname} ---")
        r=bt(sig,fwd,dm,f"{sname}_bear",rf=rf,tn=tn,pos_ratio=pr,mhd=mhd)
        r["name"]=sname
        res.append(r)
    ...
    for sname, sig1, sig2, sig3, w1, w2, rf1, rf2, tn1, tn2, pr, pr2, mhd in combo_strategies:
        ...
        if w1==0.0 and w2==0.0:
            dr=bt_series(sig1*trend_p[:, np.newaxis],fwd,dm,
                         rf=rf1,tn=tn1,pos_ratio=pr if pr is not None else trend_p,mhd=mhd)
        else:
            dr_a=bt_series(sig1,fwd,dm,rf=rf1,tn=tn1,pos_ratio=pr,mhd=mhd)
            dr_b=bt_series(sig2,fwd,dm,rf=rf2,tn=tn2,pos_ratio=pr2,mhd=mhd)
            dr=dr_a*w1+dr_b*w2
        ...
        r=eval_from_dr(dr,f"{sname}_bear")
        r["name"]=sname
        res.append(r)

    # ── 保存 ──
    with open(os.path.join(BEAR_DIR,"bear_results.json"),'w') as f:
