...
def bt_sub_strategy(sig, fwd, dm, rebal_freq=5, top_n=10, min_hold_days=10,
                    ...
                    tx_cost=TX):
    """回测单个子策略（与 comprehensive_backtest.py bt_sub_strategy 一致）
    ...
    hs = np.full(ns, -9999, dtype=np.int32)  # 建仓日期索引
    ...
    for i in range(1, n_days):
        ...
        if stop_loss_pct > 0 and np.any(pw > 0):
            for j in range(ns):
                if pw[j] > 0 and hs[j] >= 0:
                    days_held = rh - hs[j] + 1
                    cum = 1.0
                    for k in range(max(0, i - 60), i + 1):
                        if dm[k, j] and fwd[k, j] != 0:
                            cum *= (1.0 + fwd[k, j])
                    ret = cum - 1.0
                    if ret < -stop_loss_pct:
                        pw[j] = 0.0; hs[j] = -9999; entry_px[j] = 0.0; peak_px[j] = 0.0
                ...
                if pw[j] > 0 and hs[j] >= 0:
                    ...
                    for k in range(max(0, i - 60), i + 1):
                        if dm[k, j] and fwd[k, j] != 0:
                            cum *= (1.0 + fwd[k, j])
                    cur_ret = cum - 1.0
                    if cur_ret > peak_px[j] or peak_px[j] <= 0:
                        peak_px[j] = cur_ret
                    if peak_px[j] > 0 and cur_ret < peak_px[j] * (1.0 - eff_trail):
            ...
            for j in range(ns):
                if nw[j] > 0 and hs[j] < 0:
                    for k in range(i - 1, max(0, i - 5), -1):
                        if dm[k, j] and fwd[k, j] != 0:
                            entry_px[j] = max(1.0, 1.0 + fwd[k, j])
                            break
                    peak_px[j] = 0.0
                    hs[j] = rh
        ...
        if np.any(pw > 0):
            rh += 1
        prev_state = states[i] if states and i < len(states) else None
...
def run_backtest(cooldown=0, bull_thresh=0.35, min_hold=0,
    ...
    sig_s = {k: (v[si:ei+1] if hasattr(v, 'shape') and len(v.shape) >= 2 else v) for k, v in sigs.items()}
    # 修正: signals with shape (nd, ns)
    for k in ["mf", "chip", "osr", "vol_p", "trend_p", "composite_p"]:
        if k in sig_s and hasattr(sig_s[k], 'shape'):
            sig_s[k] = sig_s[k][si:ei+1]
...
def main():
    ...
    # ── 方式二: bull 广度门槛 ──────────────────────────────────────────────────
    print("\n" + "─" * 60, flush=True)
    print("方式二: bull 状态广度门槛", flush=True)
    print("─" * 60, flush=True)
    for bt in [0.40, 0.45, 0.50, 0.55, 0.60]:
    ...
    # ── 方式三: 最低持有期 ────────────────────────────────────────────────────
    print("\n" + "─" * 60, flush=True)
    print("方式三: 子策略退出最低持有期", flush=True)
    print("─" * 60, flush=True)
    for mh in [3, 5, 7, 10]:
    ...
    # ── 组合实验 ──────────────────────────────────────────────────────────────
    print("\n" + "─" * 60, flush=True)
    print("组合实验", flush=True)
    print("─" * 60, flush=True)
    combos = [
        (3, 0.45, 5), (3, 0.50, 5), (5, 0.45, 5),
        (5, 0.50, 5), (5, 0.45, 7), (3, 0.45, 3),
        (5, 0.40, 5), (5, 0.55, 5), (7, 0.45, 5),
        (10, 0.45, 5),
    ]
