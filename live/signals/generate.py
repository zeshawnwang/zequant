"""每日信号生成（MF_D10_RP / MF_D10_EMERGENCY_V1 等旧策略）。
...
Usage:
    ...
    python3 -m live.signals.generate --strategy mf_d10_rp --capital 50000 --build
"""
...
def load_weights(cfg_path):
    with open(cfg_path) as f:
        cfg = json.load(f)
    return cfg['selector']['weights'], cfg['composer']['top_n']
...
def is_rebalance_day(strategy_dir, rebal_freq, signal_date):
    ...
    if not builds:
        ...
        return True
    latest = builds[-1]
    date_str = latest.replace('build_', '').replace('.json', '')[:8]
    try:
...
def generate_orders(df, top_n, capital, holdings, strategy_name):
    ...
    alloc_per = capital / n_actual if n_actual else capital
    orders = []
    total_cost = 0.0
    for r in selected:
        sym = r['symbol']
    ...
    for sym, holding in holdings.items():
        if sym not in new_symbols:
            ...
            sell_price = round(price * 1.002, 2) if price > 0 else 0
            sell_orders.append({
...
def main():
    ...
    parser.add_argument('--build', action='store_true', help='强制建仓')
