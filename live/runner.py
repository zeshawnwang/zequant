...
用法：
    ...
    python3 -m live.runner --capital 50000        # 指定资金
"""
...
def run_step(desc: str, script_args: list, fatal: bool = False) -> bool:
    ...
    try:
        ...
        for line in result.stdout.split("\n")[-8:]:
            line = line.strip()
            if line and ("INFO" in line or "✅" in line or "❌" in line or "⚠️" in line or "║  占用" in line or "║  新占用" in line or "║  📈" in line or "║  📉" in line or "信号已写入" in line or "SMTP邮件发送" in line or "✅" in line):
...
def run_full(capital: float, extra_args: list):
    """全流程：拉数据 → 信号 → 邮件。"""
    run_step("数据更新", ["-m", "live.data_updater"], fatal=False)
...
def main():
    ...
    parser.add_argument("--mode", default="full", choices=["full", "quick", "email"],
                        help="full=全流程 quick=信号+邮件 email=只发邮件")
