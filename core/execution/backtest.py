...
from ..strategy import Order, Position, QuantStrategy
...
class BacktestEngine:
    ...
    def __init__(
        ...
        self._strategy_ref: Optional[Union[QuantStrategy, SignalStrategy]] = None
    ...
    def run(
        self,
        strategy: Union[QuantStrategy, SignalStrategy],
        ...
        for current_date in dates:
            ...
            if isinstance(strategy, SignalStrategy):
                ...
                orders = self._generate_orders_legacy(strategy, day_slice, current_date)
    ...
    def _generate_orders_legacy(
        self,
        strategy: QuantStrategy,
        day_slice: pd.DataFrame,
        current_date,
    ) -> List[Order]:
        """为QuantStrategy（旧架构）生成订单"""
        try:
            return strategy.generate_orders(day_slice, self.positions, self.cash, current_date)
        except Exception as e:
            logger.error(f"QuantStrategy生成订单失败: {e}")
            return []
