...
from core.optimization.factor_selector import FactorBacktestResult
from core.optimization.weight_optimizer import WeightConfig
...
class ConfigManager:
    ...
    def build_config(
        self,
        factor_results: List[FactorBacktestResult],
        ...
        for fr in factor_results:
            calmar = fr.report.annual_return / fr.report.max_drawdown if fr.report.max_drawdown > 0 else 0
            strategy_config.best_factors.append(FactorConfig(
                name=fr.factor_name,
                score=fr.score,
                annual_return=fr.report.annual_return,
        ...
        for wc in weight_configs:
            calmar = wc.report.annual_return / wc.report.max_drawdown if wc.report.max_drawdown > 0 else 0
            strategy_config.best_weight_configs.append(WeightConfigData(
                name=wc.name,
                weights=wc.weights,
                score=wc.score,
                annual_return=wc.report.annual_return,
