...
class RiskConstraints:
    ...
    def __init__(
        ...
        single_stock_weight: float = 0.15,    # 单只股票持仓 < 15%
        single_sector_weight: float = 0.25,   # 单个行业持仓 < 25%
        max_volatility: float = 0.30,          # 年化波动率 < 30%
        max_turnover: float = 1.00,           # 月换手率 < 100%
        min_calmar_ratio: float = 0.5,         # Calmar 比率 > 0.5
        min_win_rate: float = 0.50,           # 胜率 > 50%
    ):
